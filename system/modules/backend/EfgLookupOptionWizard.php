...
class EfgLookupOptionWizard extends Widget {
	...
	public function generate() {
		...
		$return .= '<h3><label for="' . $this->strId . '_lookup_field">' . $GLOBALS['TL_LANG'][$this->strTable]['lookup_field'][0] . '</label></h3>
		...
		$return .= '</select>';
		$return .= '
			<p class="tl_help">' . $GLOBALS['TL_LANG'][$this->strTable]['lookup_field'][1] .'</p>';

		// table field used as option value
		if ($strSelectedTable != '' && substr($strSelectedTable, 0, 3)!='fd_' )
		{
			$return .= '<h3><label for="' . $this->strId . '_lookup_val_field">' . $GLOBALS['TL_LANG'][$this->strTable]['lookup_val_field'][0] . '</label></h3>';
			...
			$return .= '</select>';
			$return .= '
				<p class="tl_help">' . $GLOBALS['TL_LANG'][$this->strTable]['lookup_val_field'][1] .'</p>';

		}


		// condition
		$return .= '<h3><label for="' . $this->strId . '_lookup_where">' . $GLOBALS['TL_LANG'][$this->strTable]['lookup_where'][0] . '</label></h3>
				<input type="text" name="' . $this->strId . '[lookup_where]" id="' . $this->strId . '_lookup_where" value="' . $this->varValue['lookup_where'] . '" class="tl_text" />';
		$return .= '
			<p class="tl_help">' . $GLOBALS['TL_LANG'][$this->strTable]['lookup_where'][1] .'</p>';
